...
(function (DR) {
  ...
  DR.page('/auth', ({ query }) => {
    ...
    if (!auth.cc || !DR.markets().includes(auth.cc)) auth.cc = DR.markets().includes(S().country) ? S().country : DR.markets()[0];
    ...
    return {
      ...
      <div class="auth pad">
        <div class="center">${DR.LOGO}<h1 class="h1 mt12">Welcome to Done Right</h1><p class="muted small">${DR.markets().length > 1 ? 'Trusted, verified services across Singapore & Malaysia' : 'Trusted, verified services across Singapore'}</p></div>
        ...
        <form id="af" class="form mt16" novalidate>
          ${auth.method === 'phone' ? DR.ui.field('Mobile number', `<div class="row gap8">${DR.markets().length > 1 ? `<select class="input w-auto" name="cc">${DR.marketCountries().map((c) => `<option value="${c.code}" ${c.code === auth.cc ? 'selected' : ''}>${c.flag} ${c.dial}</option>`).join('')}</select>` : `<span class="input w-auto dial" data-no-i18n>${C.flag} ${C.dial}</span>`}<input class="input grow" name="phone" type="tel" inputmode="numeric" autocomplete="tel-national" placeholder="${C.phoneHint}" value="${esc(auth.phone)}" ${auth.sent ? 'readonly' : ''}></div>`)
  ...
  DR.page('/settings', () => {
    ...
    return {
      ...
      <div class="list card flush">${DR.markets().length > 1 ? li('/city', 'Country / region', `<span class="muted small">${DR.COUNTRIES[s.country].flag} ${esc(DR.COUNTRIES[s.country].name)}</span>`) : ''}${btn('lang', 'Language', `<span class="muted small">${esc(DR.LANGS.find((l) => l[0] === s.lang)[1])}</span>`)}${btn('theme', 'Appearance', `<span class="muted small">${{ system: 'System', dark: 'Dark', light: 'Light' }[s.theme]}</span>`)}${btn('cache', 'Clear cached images', '<span class="muted small">2.1 MB</span>')}</div>
